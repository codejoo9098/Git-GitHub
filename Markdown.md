...
그리고 계속 언급을 하겠지만, markdown 문서는 모든 플랫폼에 항상 동일하게 적용되지 않는다. 똑같은 내용이라도 어떤 플랫폼에는 적용이 되고 어떤 플랫폼에는 적용이 안 될 수도 있다. 또한 markdown 문서는 Visual Studio Code를 이용해서 많이 볼 것이다. 하지만 Visual Studio Code를 너무 맹신해서는 안 된다. 앞서 언급한대로 VSC도 일부 명령어를 지원하지 않는 플랫폼중 하나일 수 있다. 지원하지 않을 수 있는 명령어들중 일부는 명령어 설명에 추가해놓았으니, 명령어를 사용할 때 그 부분을 유의해서 보기를 바란다.
...
학생이 쓴 md파일은 잘 읽었습니다. 정말 정성스럽게 파일을 작성한게 눈에 보여서 그 점은 정말 훌륭히 평가하고 싶었습니다. 빠진 내용 하나 없고, 각 명령어마다 예시를 붙여 놓아서 한번에 이해하기 쉬웠습니다. 게다가 주의해야 할 사항들 역시 예시를 포함해서 잘 설명했더군요. 정말 이 markdown 파일만 읽으면 다른 설명서는 읽을 필요가 없을 정도로 훌륭하게 작성했습니다. 거기다가 서술을 할 때 아무렇게나 글로 표현하지 않고 markdown의 명령어들을 적절하게 잘 이용해서 가독성 역시 매우 높았습니다. 정말 훌륭합니다. 다만 아쉬운 점도 있었습니다. 문서 자체가 매우 길어서 원하는 명령어를 찾는 것이 많이 힘들었습니다. 문서 내 링크를 다는 기능을 이용해서 원하는 명령어를 클릭했을 때, 그 명령어를 서술하는 부분으로 바로 이동할 수 있도록 했으면 어떨까 하는 아쉬움이 남습니다. 그래도 문서를 정말 정성스럽게 작성한 것이 보이고, 모든 명령어를 빠지지 않고 가독성 있게 잘 서술했기 때문에 이번 과제는 A+를 부여하겠습니다. 수고하셨습니다.!
